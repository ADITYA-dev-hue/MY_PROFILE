...
export const Hero: React.FC<HeroProps> = ({ onOpenResumeModal }) => {
  ...
  return (
    ...
    >
      {/* 4D Interactive Neural Particle Field */}
      ...
      {/* 
        MASSIVE VIBRANT CRIMSON "PORTFOLIO" TYPOGRAPHY WITH 3D MOUSE PARALLAX
      */}
      <div 
        aria-hidden="true" 
        className="absolute top-16 sm:top-12 left-0 right-0 select-none pointer-events-none w-full text-center z-0 overflow-hidden flex justify-center transition-transform duration-300 ease-out will-change-transform"
        style={{
          transform: `translate3d(${-mouseOffset.x * 0.75}px, ${-mouseOffset.y * 0.5}px, 0px)`,
        }}
      >
        <span 
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-4 items-center pt-4 sm:pt-8">
          ...
          >
            <div className="relative w-full max-w-sm sm:max-w-md">
              ...
              {/* 3D Holographic Portrait Container */}
              <HoloCard maxTilt={8} depthPop={true} className="w-full">
              ...
              <div 
                className="absolute -right-2 sm:-right-6 top-1/3 max-w-[175px] p-3 rounded-xl bg-white/95 dark:bg-black/90 border border-red-200 dark:border-red-900/80 shadow-xl backdrop-blur-md hidden sm:flex items-start gap-2 z-30 text-left transition-transform duration-200 ease-out will-change-transform"
                style={{
                  transform: `translate3d(${mouseOffset.x * 1.2}px, ${mouseOffset.y * 1.2}px, 35px)`,
                }}
              >
                <Sparkles className="w-4 h-4 text-red-600 dark:text-[#ef4444] shrink-0 mt-0.5" />
                <p className="text-[10px] text-zinc-800 dark:text-zinc-200 font-medium leading-snug">
          ...
          <motion.div 
            ...
            className="lg:col-span-3 space-y-5 text-left z-20 pt-4 lg:pt-0 pl-0 lg:pl-4"
          >
